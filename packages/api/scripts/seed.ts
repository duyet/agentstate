...
async function main() {
  ...
  console.log("");
  console.log(
    `INSERT INTO organizations (id, clerk_org_id, name, created_at) VALUES ('${ORG_ID}', 'clerk_org_local', 'Local Dev Org', ${now});`,
  );
  console.log(
    `INSERT INTO projects (id, org_id, name, slug, created_at) VALUES ('${PROJECT_ID}', '${ORG_ID}', 'My First Project', 'my-first-project', ${now});`,
  );
  console.log(
    `INSERT INTO api_keys (id, project_id, name, key_prefix, key_hash, created_at) VALUES ('${API_KEY_ID}', '${PROJECT_ID}', 'Development', '${TEST_KEY.substring(0, 12)}', '${hash}', ${now});`,
  );
  console.log("");

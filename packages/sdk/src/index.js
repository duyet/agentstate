export class AgentState {
    ...
    async request(path, options) {
        ...
        if (!res.ok) {
            ...
            throw new AgentStateError(body?.error?.message || `API error ${res.status}`, body?.error?.code || "UNKNOWN", res.status);
        }
        if (res.status === 204)
            return undefined;
        return res.json();
    ...
    async deleteState(stateKey, params) {
        return this.request(this.withQuery(`/v2/states/${encodeURIComponent(stateKey)}`, {
    ...
    async listStateEvents(stateKey, params, options) {
        return this.request(this.withQuery(`/v2/states/${encodeURIComponent(stateKey)}/events`, params), {

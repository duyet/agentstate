...
export class AgentStateCheckpointSaver extends BaseCheckpointSaver {
    ...
    async getTuple(config) {
        ...
        if (configuredCheckpointId) {
            const stateKey = createStateKey(this.stateKeyPrefix, threadId, checkpointNs, configuredCheckpointId);
            try {
        ...
        const pendingWrites = await this.parsePendingWrites(threadId, checkpointNs, ensureString(this.parseCheckpointState(state).checkpoint_id));
    ...
    async putWrites(config, writes, taskId, taskPath = "") {
        ...
        await this.client.upsertState(createWriteStateKey(this.stateKeyPrefix, threadId, checkpointNs, checkpointId, taskId), {
            agent_id: this.agentId,
            data: {
                kind: "checkpoint-writes",
                runtime: BASE_RUNTIME,

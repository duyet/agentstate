          ...
          <div className="mx-auto grid max-w-6xl items-start gap-9 px-6 py-10 pb-24 lg:grid-cols-[210px_minmax(0,1fr)] xl:grid-cols-[210px_minmax(0,1fr)_180px]">
            ...
            <article className="min-w-0 max-w-[680px]">
              ...
              <p className="mt-[14px] text-[17px] leading-[1.6] text-muted-foreground">
                AgentState is a state layer for AI agents. Store conversations, UI messages and
                graph checkpoints behind one REST API, then resume them from any runtime with a
                drop-in adapter.
              </p>
              ...
              <P>
                The core resource. Create with optional messages, append turns, then retrieve the
                full thread later with{" "}
                <code className="font-mono text-brand-ink">?include=messages</code>.
              </P>
              <EndpointList rows={CONVERSATION_ENDPOINTS} />

              <DocH id="ai-sdk">Adapters</DocH>
              <P>
                Skip the wiring. Adapters map your framework&apos;s native state shape onto
                AgentState v2 state — the same store powers Vercel AI SDK, TanStack, LangGraph and
                Cloudflare Agents.
              </P>
              ...
              <P>
                Usage is tracked per key. Query summaries and time-series for conversations,
                messages, tokens and cost.
              </P>

...
- The dashboard is served by the API Worker via the `ASSETS` binding (`packages/api/wrangler.jsonc` → `../dashboard/out`, SPA mode). `ci.yml` is the single deploy path: it builds the dashboard on PRs and on push to `main`, then deploys the Worker. The old Cloudflare Pages workflows (`dashboard-deploy.yml`, `preview-deploy.yml`) were removed because their `agentstate-dashboard` Pages project never existed (`Project not found [8000007]`), so they only produced false-red checks. Builds require `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`; use Bun (this repo ships `bun.lock` only).

## Critical Security & Deploy Gates

- **CRITICAL — dashboard-management routes auth gate (PR #132, branch `claude/fix-dashboard-api-auth`):** `/api/v1/projects*`, `/api/v1/organizations*`, `/api/v/projects*`, `/api/v/organizations*` were **completely unauthenticated** — anyone could list all projects across orgs, read any tenant's conversations/messages/analytics, and **mint or delete API keys for any project** (confirmed live). Fixed with `clerkDashboardAuth` middleware (fail-closed: 401 when `CLERK_SECRET_KEY` unset or token missing/invalid) using `@clerk/backend` `verifyToken`. `org_id` now comes from the verified Clerk session (`o_id` claim → resolved to internal org id), never the request. **DO NOT MERGE PR #132 until `CLERK_SECRET_KEY` is set in production**, or every dashboard-management route returns 401 and the dashboard breaks: `cd packages/api && wrangler secret put CLERK_SECRET_KEY` + add `CLERK_SECRET_KEY` to GitHub Actions secrets (CI deploy). Optionally `wrangler secret put CLERK_JWT_KEY` for networkless verification. After merge+deploy, verify: `curl -s -o /dev/null -w '%{http_code}' https://agentstate.app/api/v1/projects` must be **401**. Agent-facing API-key routes (conversations, `/api/projects` keys, states, leases, tokens, claims, webhooks) are untouched.
